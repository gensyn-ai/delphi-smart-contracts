...
import {IBaseTBContract} from "src/delphi/oracle/truebit/interfaces/IBaseTBContract.sol";
...
contract TruebitOracleRelayer is IBaseTBContract, UUPSUpgradeable, Ownable2StepUpgradeable, IOracle {
    ...
    constructor(address watchTower_, IDynamicParimutuelGateway gateway_, uint256 executionTimeout_, bool async_) {
        if (watchTower_ == address(0)) revert ZeroWatchTowerAddress();
        if (address(gateway_) == address(0)) revert ZeroGatewayAddress();
    ...
    function initialize(InitParams calldata p) external initializer {
        if (p.oracleFeeRecipient == address(0)) revert ZeroOracleFeeRecipient();
    ...
    function callbackTask(
        ...
        if (!IDynamicParimutuelMarket(marketProxy).isValidOutcomeIdx(outcomeIdx)) {
            GATEWAY.failMarket(marketProxy);
    ...
    function setOracleFeeRecipient(address recipient) external onlyOwner {
        if (recipient == address(0)) revert ZeroOracleFeeRecipient();
        TruebitOracleRelayerStorage storage $ = _getTruebitOracleRelayerStorage();
