...
contract LmsrMarket_TransferFrom_Test is DelphiTestUtils {
    // State Variables
    MockToken token;
    ...
    // Tests
    function testFuzz_TransferFrom_ReceiverIsMarket_Reverts(
        uint8 tokenDecimals,
        ...
        // Transfer from sender to market
        marketProxy.transferFrom({sender: sender, receiver: address(marketProxy), id: id, amount: amount});
    }

    function testFuzz_TransferFrom_Success(
        uint8 tokenDecimals,
        DelphiConfig memory delphiConfig,
        ILmsrMarket.MarketConfig memory marketConfig,
        uint256 initialDeposit,
        ...
        .quoteBuyExactOut({marketProxy: marketProxy, outcomeIdx: outcomeIdx, sharesOut: sharesOut}) returns (
            ...
            // Sender approves shares out to receiver
            marketProxy.approve({spender: receiver, id: outcomeIdx, amount: sharesOut});
            ...
            // Switch to receiver
            _useNewSender(receiver);

            // Sender transfers shares to receiver
            marketProxy.transferFrom({sender: sender, receiver: receiver, id: outcomeIdx, amount: sharesOut});
