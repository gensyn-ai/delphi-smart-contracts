...
contract LmsrMarket_TotalSupply_Test is DelphiTestUtils {
    // State Variables
    MockToken token;
    ...
    function testFuzz_TotalSupply_OutcomeIsOutOfBounds_ReturnsZero(
        uint8 tokenDecimals,
        ...
        // Buy shares (so a valid outcome has nonzero supply, distinguishing "invalid" from "empty")
        ...
        // Sanity check: the bought outcome has the bought supply
        assertEq(marketProxy.totalSupply(boughtOutcomeIdx), sharesOut, "valid outcome supply mismatch");
        ...
        // An unconfigured outcome id returns zero supply (no revert)
        assertEq(marketProxy.totalSupply(invalidOutcomeIdx), 0, "invalid outcome supply != 0");
    }
}
