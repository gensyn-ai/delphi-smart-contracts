...
import {IWatchTower} from "src/delphi/oracle/truebit/interfaces/IWatchTower.sol";
...
import {DOTypes} from "src/delphi/oracle/truebit/abstract/Types.sol";

contract TruebitOracleRelayer_Test is DelphiTestUtils, DelphiDeployer, TruebitOracleRelayerDeployer {
    // Actors
    ...
    function _setUp() private {
        ...
        gateway = delphi.dynamicParimutuelGateway;
        implementation = delphi.dynamicParimutuelImplementation;

        // Deploy TruebitOracleRelayer impl + proxy
        TruebitOracleRelayer impl = new TruebitOracleRelayer(
            WATCH_TOWER, IDynamicParimutuelGateway(address(gateway)), EXECUTION_TIMEOUT, ASYNC
        );
    ...
    function _setUpMarketAwaitingSettlement() private returns (IDynamicParimutuelMarket marketProxy) {
        _setUp();
        return _createMarketAwaitingSettlement();
    }
    ...
    function _createMarketAwaitingSettlement() private returns (IDynamicParimutuelMarket marketProxy) {
        ...
        marketProxy = IDynamicParimutuelMarket(
            gateway.delphiFactory()
                ...
                newMarketInitializationCalldata: abi.encode(config)
            })
        );
    ...
    function test_SetOracleFeeRecipient_Reverts_ZeroAddress() external {
        ...
        _useNewSender(OWNER);
        vm.expectRevert(TruebitOracleRelayer.ZeroOracleFeeRecipient.selector);
    ...
    function test_Initialize_Reverts_ZeroOracleFeeRecipient() external {
        ...
        TruebitOracleRelayer impl = new TruebitOracleRelayer(
            WATCH_TOWER, IDynamicParimutuelGateway(makeAddr("gateway")), EXECUTION_TIMEOUT, ASYNC
        );
    ...
    function test_Constructor_Reverts_ZeroWatchTower() external {
        ...
        new TruebitOracleRelayer(address(0), IDynamicParimutuelGateway(makeAddr("gateway")), EXECUTION_TIMEOUT, ASYNC);
    }
    ...
    function deployReusedExternal(
        address watchTower,
        IDynamicParimutuelGateway gateway_,
    ...
    function test_DeployReusedImpl_Succeeds_WhenConfigMatches() external {
        IDynamicParimutuelGateway gw = IDynamicParimutuelGateway(makeAddr("gateway"));
    ...
    function test_DeployReusedImpl_Reverts_GatewayMismatch() external {
        IDynamicParimutuelGateway gw = IDynamicParimutuelGateway(makeAddr("gateway"));
        IDynamicParimutuelGateway otherGateway = IDynamicParimutuelGateway(makeAddr("OTHER_GATEWAY"));
    ...
    function test_DeployReusedImpl_Reverts_AsyncMismatch() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TruebitOracleRelayerDeployer.ImplementationAsyncMismatch.selector, ASYNC, otherAsync
            )
        );
        this.deployReusedExternal(WATCH_TOWER, gw, EXECUTION_TIMEOUT, ASYNC, address(impl), _matchingInitParams());
    }
    ...
    function test_SetOracleFeeRecipient_NewRecipientReceivesFee() external {
        ...
        DynamicParimutuelGateway _gateway = delphi.dynamicParimutuelGateway;
        DynamicParimutuelMarket _impl = delphi.dynamicParimutuelImplementation;

        TruebitOracleRelayer _relayer = TruebitOracleRelayer(
            address(
                new ERC1967Proxy(
                    address(
                        new TruebitOracleRelayer(
                            WATCH_TOWER, IDynamicParimutuelGateway(address(_gateway)), EXECUTION_TIMEOUT, ASYNC
                        )
                    ),
                    abi.encodeCall(
                        TruebitOracleRelayer.initialize,
        ...
        IDynamicParimutuelMarket marketProxy = IDynamicParimutuelMarket(
            _gateway.delphiFactory()
                ...
                initialDeposit_: initialDeposit,
                newMarketMetadata: IDelphiMarket.VerifiableUri({uri: "uri", uriContentHash: keccak256("uri")}),
                newMarketInitializationCalldata: abi.encode(
    ...
    function test_ResolveMarket_BodyExactFormat() external {
        ...
        // fails here with a clear message instead of an opaque calldata mismatch.
        ...
        assertEq(marketProxy.getMarket().config.tradingDeadline, 121, "test assumes tradingDeadline == 121");

        // Hardcoded expected body — NOT built via the contract's string.concat.
        string memory expectedBody = '{"metadata_path": "uri", "created_at": 1, "resolves_at": 121}';
    ...
    function test_ResolveMarket_Reverts_CallerNotGateway(address caller) external {
        IDynamicParimutuelMarket marketProxy = _setUpMarketAwaitingSettlement();
        ...
        _useNewSender(caller);
        vm.expectRevert(abi.encodeWithSelector(TruebitOracleRelayer.CallerNotGateway.selector, caller));
    ...
    function test_ResolveMarket_Reverts_DuplicateExecutionId() external {
        IDynamicParimutuelMarket marketA = _setUpMarketAwaitingSettlement();
        IDynamicParimutuelMarket marketB = _createMarketAwaitingSettlement();
    ...
    function test_FullSettlement_DisbursesBothFees() external {
        _useNewSender(OWNER);
        uint256 keeperFee = 1_000_000; // 1 USDC
        uint256 oracleFee = 1_000_000; // 1 USDC
        ...
        DelphiAddresses memory delphi = _deployDelphi(
            DelphiConfig({
                tradingFeesRecipient: OWNER,
                marketCreationFeeRecipient: OWNER,
                marketCreationFee: keeperFee + oracleFee, // must cover both settlement fees
                keeperFee: keeperFee,
                oracleFee: oracleFee,
                tradingFeesRecipientPct: TRADING_FEES_PCT,
                token: _token,
        ...
        _useNewSender(CREATOR);
        uint256 initialDeposit = _impl.MIN_INITIAL_DEPOSIT();
        deal(address(_token), CREATOR, initialDeposit + keeperFee + oracleFee);
        _token.approve(address(_gateway.delphiFactory()), initialDeposit + keeperFee + oracleFee);
        ...
        address keeper = makeAddr("keeper");
        uint256 keeperBefore = _token.balanceOf(keeper);
        uint256 recipientBefore = _token.balanceOf(ORACLE_FEE_RECIPIENT);
        ...
        _useNewSender(keeper);
        _gateway.resolveMarket(address(marketProxy));
        assertEq(_token.balanceOf(keeper) - keeperBefore, keeperFee, "keeper should receive KEEPER_FEE at resolve");
        ...
        assertEq(
            _token.balanceOf(ORACLE_FEE_RECIPIENT) - recipientBefore,
            oracleFee,
    ...
    function test_Upgrade_PreservesStorageAndOwner() external {
        ...
        TruebitOracleRelayerV2 implV2 = new TruebitOracleRelayerV2(
            WATCH_TOWER, IDynamicParimutuelGateway(address(gateway)), EXECUTION_TIMEOUT, ASYNC
        );
        _useNewSender(OWNER);
    ...
    /// @dev Ownership transfer is two-step (Ownable2StepUpgradeable): transferOwnership only stages the
    ///      pending owner; it does not take effect until the pending owner calls acceptOwnership. This
    ///      prevents a fat-fingered transferOwnership from locking admin.
    function test_Ownable2Step_TransferOwnership() external {
        ...
        assertEq(relayer.pendingOwner(), newOwner, "pendingOwner should be set");
        assertEq(relayer.owner(), OWNER, "owner should not change until accepted");

        _useNewSender(newOwner);
        relayer.acceptOwnership();
        assertEq(relayer.owner(), newOwner, "owner should be updated after acceptance");
        assertEq(relayer.pendingOwner(), address(0), "pendingOwner should be cleared");
    }

    function test_Ownable2Step_Reverts_AcceptByNonPendingOwner(address caller) external {
        _setUp();
...
contract TruebitOracleRelayerV2 is TruebitOracleRelayer {
    ...
    constructor(address watchTowerAddress, IDynamicParimutuelGateway gateway_, uint256 executionTimeout_, bool async_)
